...
def configure_environment(env=None):
    ...
    threads = positive(env.get("B5_NUM_THREADS", env.get("OMP_NUM_THREADS", "4")), "B5_NUM_THREADS")
    env["B5_NUM_THREADS"] = str(threads)
    for key in THREAD_KEYS:
        env[key] = str(threads)
    env["B5_IO_WORKERS"] = str(positive(env.get("B5_IO_WORKERS", threads), "B5_IO_WORKERS"))
    return threads
