...
def verify_release(release):
    ...
    runtime_sources = {"source/" + name for name in
                       ("perception_runtime.py", "perception_workers.py", "runtime_settings.py", "ordered_prefetch.py", "sam2_episode_cache.py", "prepare_sam_cache.py")}
...
def command_for(release, output, cfg, effective, base, variant, reference_full=None):
    ...
    cmd = [sys.executable, "-u", "-B", str(output / "source/3-train_evaluation.py"),
           ...
           *(str(Path(paths["RESULT_ROOT"]) / base / (base+c)) for c in CONDITIONS),
           ...
           "--sam2_cache_root", paths["SAM2_CACHE_ROOT"]]
        ...
        for base, obj in TEST.items():
            ...
            for c in CONDITIONS:
                ...
                episode_rows.append(dict(variant=variant, base_sequence=base, object_id=obj, episode=seq,
                    ...
                    measured_cached_path_ms=float(df["policy_wall_ms"].mean()),
                    runtime_scope="cached_masks_excludes_sam2_preprocessing_not_online_fps",
...
def main(argv=None):
    ...
    if 'sam2_cache_index.json' not in {line.split('  ', 1)[1] for line in
            (release/'freeze.sha256').read_text().splitlines()}:
        raise ValueError('Release lacks frozen SAM2 cache index')
    cache_index = verify_index(release/'sam2_cache_index.json')
    if cache_index['manifest_sha256'] != sha(release/'reference_manifest.csv'):
        raise ValueError('SAM2 cache manifest mismatch')
