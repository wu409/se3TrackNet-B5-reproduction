...
SAM2 advances exactly once per manifest frame, before SE3/B5 consumes that frame.
GPU work is serialized (resident models are NOT unloaded). Worker failure is a
...
CONFIG = dict(version="persistent_perception_v1", sam2="one_forward_generator_per_episode",
              ...
              cache="current_mask_only_no_cross_run_cache", timeout_seconds=1200,
...
class PerceptionSession:
    def __init__(self, args, rgb_paths, initial_mask_path, mesh_file, log_prefix):
        ...
        self.sam = JsonWorker(args.sam2_python, "sam2", sam_cfg, self.scratch, str(prefix) + "_sam2.log")
    ...
    def current_mask(self, rgb_paths, initial_mask_path):
        ...
        diagnostics = dict(sam2_called=True, sam2_subprocess_started=False,
            sam2_cache_hit=False, sam2_persistent=True, sam2_current_frame_reused=True,
            sam2_target_index=self.index, sam2_frames_supplied=self.index + 1,
            sam2_propagated_frames=self.index + 1, sam2_mask_pixels=pixels,
    ...
    def close(self, complete=False):
        ...
        try:
            ...
            self.summary_path.write_text(json.dumps(dict(config=CONFIG, execution_settings=runtime_settings.execution_config(), complete=bool(complete),
                ...
                sam2_log=str(self.sam.log_path), foundationpose_log=str(self.fp.log_path)), indent=2), encoding="utf-8")
        finally:
            _ACTIVE = None
            self.temp.cleanup()
