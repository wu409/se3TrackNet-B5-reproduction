...
class ScorePredictor:
  ...
  def predict(
      ...
      """
      Low-VRAM score prediction.

      All pose hypotheses are preserved, but they are processed
      in small chunks to avoid constructing all RGB/depth/XYZ crops
      ...
      # Keep original observation tensors on GPU only once.
      rgb = torch.as_tensor(
          rgb,
          device="cuda",
          dtype=torch.float
      ...
      # ---------------------------------------------------------
      # IMPORTANT:
      # 4 GB GPU -> start with 2.
      # If stable, you may increase to 4.
      # ---------------------------------------------------------
      CHUNK_SIZE = 2

      num_poses = len(ob_in_cams)

      logging.info(
          f"Scoring {num_poses} pose hypotheses "
      ...
      # Keep poses on CPU here.
      for start in range(0, num_poses, CHUNK_SIZE):
          ...
          logging.info(
              f"Processing score chunk "
              f"{start}:{end}/{num_poses}"
          )

          poses_chunk = ob_in_cams[start:end]

          # -----------------------------------------------------
          # Only this small group of hypotheses enters CUDA crop
          # generation / rendering / xyz-map construction.
          # -----------------------------------------------------
          ...
          # -----------------------------------------------------
          # Build ScoreNet inputs for THIS CHUNK only
          # -----------------------------------------------------
          A = torch.cat(
              [
                  pose_data.rgbAs.cuda(),
                  pose_data.xyz_mapAs.cuda()
              ],
              dim=1
          ).float()
          ...
          B = torch.cat(
              [
                  pose_data.rgbBs.cuda(),
                  pose_data.xyz_mapBs.cuda()
              ],
              dim=1
          ).float()
          ...
          # Move score to CPU temporarily so GPU memory is released.
          all_scores.append(
              scores_chunk.detach().cpu()
          )

          # Optional debug visualization.
          # Normally run_demo.py debug=1 -> get_vis=False,
          # therefore this branch costs nothing.
          if get_vis:
              ...
              canvas_chunk = vis_batch_data_scores(
                  pose_data,
                  ids=local_ids,
                  scores=scores_chunk
              )
          ...
          # -----------------------------------------------------
          # Explicitly release this chunk before processing next
          # -----------------------------------------------------
          del A
          del B
          del output
      ...
      # ---------------------------------------------------------
      # Reassemble ALL hypothesis scores in original order
      # ---------------------------------------------------------
      scores = torch.cat(
          all_scores,
          dim=0
      ...
      # Keep same +100 offset used in the original implementation.
      scores = scores + 100.0

      logging.info(
          f"forward done, scores shape: {scores.shape}"
      )
